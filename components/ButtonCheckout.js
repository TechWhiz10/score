...
const ButtonCheckout = ({ priceId, mode = "subscription" }) => {
  ...
  return (
    <button
      className="btn btn-primary btn-block flex items-center justify-center px-4 py-2 text-white font-semibold whitespace-nowrap"
      onClick={handlePayment}
    >
      {isLoading ? (
        <span className="loading loading-spinner loading-xs"></span>
      ) : (
        <span>Bet With {config?.appName}</span>
      )}
    </button>
  );
};
